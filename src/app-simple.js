...
app.get('/', (req, res) => {
  res.json({
    ...
    note: 'Para usar la base de datos, ejecuta el script SQL en Render'
  });
});
...
function startServer() {
  try {
    // Iniciar servidor sin verificar base de datos
    app.listen(PORT, () => {
      console.log(`🚀 Servidor ejecutándose en puerto ${PORT}`);
      console.log(`📱 API disponible en: http://localhost:${PORT}`);
      console.log(`🏥 Health check: http://localhost:${PORT}/health`);
      console.log(`📚 Documentación: http://localhost:${PORT}/`);
      console.log(`⚠️  Nota: Para usar la base de datos, ejecuta el script SQL en Render`);
    });
  } catch (error) {
